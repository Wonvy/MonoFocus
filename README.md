# MonoFocus

<div align="center">

**跨平台多显示器护眼与专注工具**

![License](https://img.shields.io/badge/license-MIT-blue.svg)
![Platform](https://img.shields.io/badge/platform-Windows%20%7C%20macOS%20%7C%20Linux-lightgrey)
![Rust](https://img.shields.io/badge/rust-1.70%2B-orange)
![Tauri](https://img.shields.io/badge/tauri-1.5%2B-blue)

</div>

---

## 📖 简介

**MonoFocus** 是一款基于 Rust + Tauri 开发的跨平台多显示器护眼工具。当您的鼠标移动到某一台显示器时，其他显示器会自动显示半透明黑色遮罩，降低视觉干扰，帮助您专注于当前工作区域，同时减少多屏环境下的眼部疲劳。

## 📸 软件截图

### 主界面
![主界面](screenshots/main.png)

### ✨ 核心特性

- 🖥️ **多显示器智能检测** - 自动识别所有已连接的显示器
- 🎯 **实时跟随鼠标** - 智能判断当前活跃显示器
- 🌓 **动态遮罩层** - 非活跃显示器自动显示半透明遮罩
- 🎚️ **透明度可调** - 支持 0-80% 透明度自由调节
- 🎨 **显示器可视化** - 以真实比例展示所有显示器布局
- 💾 **配置持久化** - 自动保存您的偏好设置
- 🚀 **轻量高效** - 基于 Rust 构建，资源占用极低
- 🎯 **点击穿透** - 遮罩层不影响正常操作
- 🔔 **系统托盘** - 最小化到托盘，随时快速切换
- 🌍 **跨平台支持** - Windows、macOS、Linux 一致体验

---

## 🚀 快速开始

### 安装

#### 从 Release 下载（推荐）

前往 [Releases](https://github.com/yourusername/MonoFocus/releases) 页面下载对应平台的安装包：

- **Windows**: `MonoFocus_x64_setup.exe`
- **macOS**: `MonoFocus.dmg`
- **Linux**: `MonoFocus_amd64.AppImage` / `MonoFocus_amd64.deb`

#### 从源码构建

```bash
# 克隆仓库
git clone https://github.com/yourusername/MonoFocus.git
cd MonoFocus

# 安装依赖
npm install

# 开发模式
npm run tauri dev

# 构建生产版本
npm run tauri build
```

### 系统要求

- **操作系统**: Windows 10+, macOS 10.15+, 主流 Linux 发行版
- **运行时**: 无需额外依赖（Tauri 打包为独立可执行文件）

---

## 🎨 功能说明

### 1. 显示器布局可视化

MonoFocus 的主界面会以缩略图形式展示所有检测到的显示器：

- **真实比例渲染**：根据物理尺寸或分辨率计算相对大小
- **位置关系还原**：保持系统中配置的显示器排列（左右、上下）
- **当前屏高亮**：鼠标所在显示器会以高亮边框标识
- **极简设计**：无文字标签，纯视觉化展示

### 2. 智能遮罩

- 鼠标移动到任何显示器时，其他屏幕自动覆盖半透明黑色遮罩
- 遮罩窗口始终置顶，但允许鼠标和键盘事件穿透
- 支持实时调整透明度（0-80%）

### 3. 系统托盘

右键托盘图标可快速访问：

- **Enable / Disable Shield** - 快速开关护眼模式
- **Settings** - 打开设置窗口
- **Exit** - 退出应用

### 4. 配置管理

所有设置自动保存，包括：
- 遮罩透明度
- 护眼模式启用状态
- 开机自启动选项（可选）

---

## 🛠️ 技术架构

### 技术栈

- **后端**: Rust 1.70+
- **框架**: Tauri 1.5+
- **前端**: HTML5 / CSS3 / JavaScript（可扩展 Vue/React）
- **配置存储**: tauri-plugin-store / JSON

### 架构图

```
┌─────────────────────────────────────────────────────────┐
│                     Tauri Frontend                      │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐ │
│  │ 显示器缩略图  │  │ 透明度滑条   │  │ 模式开关     │ │
│  └──────────────┘  └──────────────┘  └──────────────┘ │
└─────────────────────────────────────────────────────────┘
                           ↕ IPC (invoke)
┌─────────────────────────────────────────────────────────┐
│                      Rust Backend                       │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐ │
│  │ monitor.rs   │  │ mouse_       │  │ overlay.rs   │ │
│  │ 显示器检测   │  │ watcher.rs   │  │ 遮罩窗口管理 │ │
│  └──────────────┘  └──────────────┘  └──────────────┘ │
│  ┌──────────────┐  ┌──────────────┐                   │
│  │ config.rs    │  │ tray.rs      │                   │
│  │ 配置管理     │  │ 托盘图标     │                   │
│  └──────────────┘  └──────────────┘                   │
└─────────────────────────────────────────────────────────┘
```

### 核心模块

#### 1. `monitor.rs` - 显示器检测

```rust
pub struct MonitorInfo {
    pub id: String,              // 显示器唯一标识
    pub x: i32,                  // 虚拟桌面 X 坐标
    pub y: i32,                  // 虚拟桌面 Y 坐标
    pub width: i32,              // 逻辑宽度（像素）
    pub height: i32,             // 逻辑高度（像素）
    pub physical_width_mm: Option<f32>,  // 物理宽度（毫米）
    pub physical_height_mm: Option<f32>, // 物理高度（毫米）
    pub scale_factor: f32,       // HiDPI 缩放比例
}
```

**功能**：
- 跨平台获取所有显示器信息
- 提取物理尺寸（用于真实比例渲染）
- 监听显示器连接/断开事件

**平台实现**：
- **Windows**: `EnumDisplayMonitors` + `GetMonitorInfo`
- **macOS**: `CGDisplayBounds` + `CGDisplayScreenSize`
- **Linux**: X11 `XRRGetMonitors` / Wayland `wl_output`

#### 2. `mouse_watcher.rs` - 鼠标监听

**功能**：
- 实时获取鼠标全局坐标
- 判断鼠标所在的显示器
- 触发遮罩层更新事件

**平台实现**：
- **Windows**: `GetCursorPos` + `MonitorFromPoint`
- **macOS**: `CGEvent::location` + 坐标匹配
- **Linux**: X11 `XQueryPointer` / Wayland 事件监听

#### 3. `overlay.rs` - 遮罩窗口

**功能**：
- 为每个非活跃显示器创建全屏透明窗口
- 设置窗口属性：无边框、透明、点击穿透、置顶
- 动态调整透明度和显示/隐藏状态

**关键技术**：
- **Windows**: 
  - `WS_EX_LAYERED` + `WS_EX_TRANSPARENT` 实现透明穿透
  - `SetLayeredWindowAttributes` 设置透明度
- **macOS**:
  - `NSWindowStyleMaskBorderless` + `ignoresMouseEvents`
  - `alphaValue` 控制透明度
- **Linux**:
  - X11: `_NET_WM_WINDOW_TYPE_DOCK` + `XShapeCombineRectangles`
  - Wayland: Layer Shell Protocol

#### 4. `monitor_layout_mapper.rs` - 布局计算

**功能**：
- 将真实显示器坐标映射到 UI 容器
- 保持相对位置关系和宽高比例
- 提供标准化布局数据给前端

**算法示例**：

```rust
pub fn normalize_layout(monitors: &[MonitorInfo], container_width: f32, container_height: f32) -> Vec<UIRect> {
    // 1. 计算包络盒
    let min_x = monitors.iter().map(|m| m.x).min().unwrap();
    let max_x = monitors.iter().map(|m| m.x + m.width).max().unwrap();
    let min_y = monitors.iter().map(|m| m.y).min().unwrap();
    let max_y = monitors.iter().map(|m| m.y + m.height).max().unwrap();
    
    let total_width = (max_x - min_x) as f32;
    let total_height = (max_y - min_y) as f32;
    
    // 2. 计算缩放比例（保持纵横比）
    let margin = 20.0;
    let scale = ((container_width - 2.0 * margin) / total_width)
        .min((container_height - 2.0 * margin) / total_height);
    
    // 3. 映射每个显示器
    monitors.iter().map(|m| {
        UIRect {
            x: (m.x - min_x) as f32 * scale + margin,
            y: (m.y - min_y) as f32 * scale + margin,
            width: m.width as f32 * scale,
            height: m.height as f32 * scale,
            id: m.id.clone(),
        }
    }).collect()
}
```

#### 5. `config.rs` - 配置管理

**存储位置**：
- Windows: `%APPDATA%\MonoFocus\config.json`
- macOS: `~/Library/Application Support/MonoFocus/config.json`
- Linux: `~/.config/MonoFocus/config.json`

**配置结构**：

```json
{
  "opacity": 0.6,
  "enabled": true,
  "auto_start": false,
  "theme": "auto"
}
```

---

## 🎯 使用场景

- **多屏开发者** - 专注主显示器代码，减少其他屏幕干扰
- **设计师** - 在设计时隐藏参考屏幕的视觉噪音
- **交易员** - 聚焦当前监控的数据面板
- **内容创作者** - 降低多屏环境的视觉疲劳
- **长时间电脑用户** - 护眼模式减轻眼部压力

---

## 🔧 开发指南

详细开发文档请参阅：[DEVELOPMENT.md](./DEVELOPMENT.md)

### 项目结构

```
MonoFocus/
├── src-tauri/              # Rust 后端
│   ├── src/
│   │   ├── main.rs         # 入口文件
│   │   ├── monitor.rs      # 显示器检测
│   │   ├── mouse_watcher.rs
│   │   ├── overlay.rs      # 遮罩窗口
│   │   ├── config.rs       # 配置管理
│   │   └── tray.rs         # 系统托盘
│   └── Cargo.toml
├── src/                    # 前端源码
│   ├── index.html
│   ├── styles.css
│   ├── main.js
│   └── components/
│       ├── MonitorLayout.js
│       └── OpacitySlider.js
├── public/                 # 静态资源
├── icons/                  # 应用图标
├── package.json
└── README.md
```

### 本地开发

```bash
# 安装依赖
npm install

# 启动开发服务器（带热重载）
npm run tauri dev

# 代码格式化
cargo fmt --manifest-path=src-tauri/Cargo.toml

# 运行测试
cargo test --manifest-path=src-tauri/Cargo.toml

# 构建
npm run tauri build
```

---

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！

1. Fork 本仓库
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启 Pull Request

---

## 📄 许可证

本项目采用 MIT 许可证 - 详见 [LICENSE](LICENSE) 文件

---

## 🙏 致谢

- [Tauri](https://tauri.app/) - 跨平台应用框架
- [Rust](https://www.rust-lang.org/) - 系统编程语言
- 所有贡献者和用户

---

## 📞 联系方式

- **问题反馈**: [GitHub Issues](https://github.com/yourusername/MonoFocus/issues)
- **讨论交流**: [GitHub Discussions](https://github.com/yourusername/MonoFocus/discussions)

---

<div align="center">

**如果这个项目对您有帮助，请给一个 ⭐ Star！**

Made with ❤️ by MonoFocus Team

</div>

